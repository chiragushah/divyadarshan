// DivyaDarshanam - Rewrite ExploreClient.tsx with correct UTF-8
// Run: node fix-explore.js
const fs = require('fs')
...
// All emojis as unicode escapes - cannot be corrupted by Windows
const PIN    = '\uD83D\uDCCD'  // 📍
const TEMPLE = '\uD83D\uDED5'  // 🛕
const MAP    = '\uD83D\uDDFA\uFE0F' // 🗺️
const CANDLE = '\uD83E\uDE94'  // 🪔
...
const TABS = [
  ...
  { id: 'darshan',   label: '${RED} Live Darshan' },
  { id: 'nearby',    label: '${PIN} Nearby' },
  { id: 'seasonal',  label: '${CAL} This Month' },
]
          ...
          <div>
            {locLoading && (
              <div className="flex flex-col items-center justify-center py-20 gap-4">
                <div style={{ fontSize: 40 }}>${PIN}</div>
                <p className="text-sm font-semibold" style={{ color: 'var(--ink)' }}>Finding sacred places near you${ELLIP}</p>
                <p className="text-xs" style={{ color: 'var(--muted)' }}>Hindu temples ${DOT} Jain temples ${DOT} Gurudwaras ${DOT} Buddhist sites</p>
              </div>
            )}
            {locationError && (
              ...
              <>
                <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
                  <div className="flex items-center gap-2">
                    <span style={{ fontSize: 20 }}>${PIN}</span>
                    <div>
                      <p className="text-sm font-semibold" style={{ color: 'var(--ink)' }}>{nearbyTemples.length} sacred places found</p>
                      <p className="text-xs" style={{ color: 'var(--muted)' }}>Within {radiusKm}km ${DOT} via OpenStreetMap</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                {nearbyTemples.length === 0 ? (
                  <div className="text-center py-16">
                    <div style={{ fontSize: 40, marginBottom: 12 }}>${TEMPLE}</div>
                    ...
                    {nearbyTemples.map((temple: any) => (
                      <div key={temple.id} className="card card-p flex items-start gap-4" style={{ borderColor: 'var(--border)' }}>
                        <div className="flex-shrink-0 flex flex-col items-center justify-center rounded-xl"
                          ...
                          <span style={{ fontSize: 20 }}>${TEMPLE}</span>
          ...
          <div>
            {monthFestivals.map((f, i) => (
              <div key={i} className="rounded-2xl p-5 mb-6 flex gap-4 items-start"
                ...
                <div style={{ fontSize:36, flexShrink:0 }}>${CANDLE}</div>
                <div>
                  <div className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color:'var(--saffron)' }}>
                    {new Date().toLocaleString('default',{month:'long'})} ${DOT} Auspicious Occasion
                  </div>
          ...
          <>
            ...
            <div className="flex items-center justify-between mb-5">
              <p className="text-sm" style={{ color:'var(--muted)' }}>
                {activeFilters.category && <><strong style={{ color:'var(--crimson)' }}>{activeFilters.category}</strong> ${DOT} </>}
                {total.toLocaleString()} temples
                {activeTab==='darshan' && <span className="ml-2" style={{ color:'var(--live)' }}>${RED} Live only</span>}
              </p>
...
const outPath = path.join(P, 'app/(app)/explore/ExploreClient.tsx')
fs.writeFileSync(outPath, content, 'utf8')
console.log('OK: ExploreClient.tsx written')

// Push
process.chdir(P)
try {
  execSync('git add "app/(app)/explore/ExploreClient.tsx"', { stdio: 'inherit' })
  execSync('git commit -m "fix: rewrite ExploreClient with correct UTF-8 emojis"', { stdio: 'inherit' })
