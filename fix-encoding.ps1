# DivyaDarshan - Fix emoji encoding + Google Translate position
# Uses UTF8 without BOM so emojis render correctly

$PROJECT = "C:\Users\chira\Downloads\divyadarshan"

Write-Host "============================================" -ForegroundColor Cyan
Write-Host " DivyaDarshan - Encoding + Translate Fix" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
...
# Helper: write UTF-8 WITHOUT BOM (fixes emoji rendering)
...
# ─────────────────────────────────────────────────────────────
# FIX 1+2: ExploreClient.tsx — re-write with correct encoding
# ─────────────────────────────────────────────────────────────
Write-Host "`n[1/3] Re-writing ExploreClient.tsx (UTF8 no BOM)..." -ForegroundColor Yellow
      ...
      try {
        ...
        if (fallbackData.temples?.length > 0) {
          const normalized = fallbackData.temples.map((t: any) => ({
            ...t, lon: t.lon ?? t.lng, icon: '[Hindu]', religion: 'Hindu',
          }))
            ...
            {locationError && (
              <div className="card card-p text-center py-12">
                ...
                <button className="btn btn-primary" onClick={() => {
                  setUserCoords(null); setNearbyTemples([]); setLocationError(''); setRetryKey(k => k + 1)
                }}>Try Again</button>
              </div>
            )}
...
WriteUTF8NoBOM "app\(app)\explore\ExploreClient.tsx" $exploreClient
Write-Host "   OK" -ForegroundColor Green

# ─────────────────────────────────────────────────────────────
# FIX: GoogleTranslate.tsx
# - Skip on auth pages
# - Hide the widget completely (it lives in Navbar already)
#   so it stops floating at top of page
# ─────────────────────────────────────────────────────────────
Write-Host "`n[2/3] Re-writing GoogleTranslate.tsx (fix position + encoding)..." -ForegroundColor Yellow
...
// Skip translate on auth pages - Google Translate rewrites DOM
// including hidden CSRF inputs which breaks NextAuth sign-in
...
export default function GoogleTranslate() {
  ...
  useEffect(() => {
    if (SKIP_PATHS.some(p => pathname?.startsWith(p))) {
      ...
      // Clear googtrans cookie so it does not interfere with NextAuth
  ...
  return (
    <>
      ...
      <style>{`
        ...
        /* CRITICAL: prevent Google from pushing the whole page down */
...
WriteUTF8NoBOM "components\translate\GoogleTranslate.tsx" $googleTranslate
Write-Host "   OK" -ForegroundColor Green
...
git commit -m "fix: UTF8 no BOM encoding (emoji fix) + Google Translate position"
git push
