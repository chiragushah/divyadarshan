// build-contribution-system.js
const fs = require('fs')

// ── 1. MongoDB Model ─────────────────────────────────────────────────────────
fs.mkdirSync('models', { recursive: true })
fs.writeFileSync('models/Contribution.ts', `import { Schema, model, models, Document } from 'mongoose'
...
`)
console.log('✅ models/Contribution.ts')

// ── 2. API Route ──────────────────────────────────────────────────────────────
fs.mkdirSync('app/api/contribution', { recursive: true })
fs.writeFileSync('app/api/contribution/route.ts', `import { NextRequest, NextResponse } from 'next/server'
...
`)
console.log('✅ app/api/contribution/route.ts')

// ── 3. Contribution Modal Component ──────────────────────────────────────────
fs.writeFileSync('components/ContributionModal.tsx', `'use client'
...
`)
console.log('✅ components/ContributionModal.tsx')

// ── 4. Contribution Banner ────────────────────────────────────────────────────
fs.writeFileSync('components/ContributionBanner.tsx', `'use client'
import { useState } from 'react'
...
`)
console.log('✅ components/ContributionBanner.tsx')

// ── 5. Add banner to app layout ───────────────────────────────────────────────
const appLayoutPath = 'app/(app)/layout.tsx'
if (fs.existsSync(appLayoutPath)) {
  let layout = fs.readFileSync(appLayoutPath, 'utf8')
  if (!layout.includes('ContributionBanner')) {
    layout = layout.replace(
      "import Navbar from '@/components/nav/Navbar'",
      "import Navbar from '@/components/nav/Navbar'\nimport ContributionBanner from '@/components/ContributionBanner'"
    )
    // Add before closing tag
    layout = layout.replace('</>', '<ContributionBanner />\n    </>')
    layout = layout.replace('</div>', '<ContributionBanner />\n  </div>', )
    // Simpler: add before last closing
    if (!layout.includes('ContributionBanner />')) {
      layout = layout.replace(/(<\/(?:div|main|section)>\s*\))/m, '<ContributionBanner />\n$1')
    }
    fs.writeFileSync(appLayoutPath, layout, 'utf8')
    console.log('✅ app/(app)/layout.tsx: ContributionBanner added')
  } else {
    console.log('⏭️  ContributionBanner already in app layout')
  }
}

// ── 6. Add banner to public layout ───────────────────────────────────────────
const pubLayoutPath = 'app/(public)/layout.tsx'
if (fs.existsSync(pubLayoutPath)) {
  let layout = fs.readFileSync(pubLayoutPath, 'utf8')
  if (!layout.includes('ContributionBanner')) {
    if (layout.includes('import')) {
      layout = layout.replace(
        /^(import .+\n)/m,
        "$1import ContributionBanner from '@/components/ContributionBanner'\n"
      )
    }
    layout = layout.replace(/(<\/(?:div|main)>\s*\))/m, '<ContributionBanner />\n$1')
    fs.writeFileSync(pubLayoutPath, layout, 'utf8')
    console.log('✅ app/(public)/layout.tsx: ContributionBanner added')
  }
}

// ── 7. Add nodemailer if not already installed (package.json check) ───────────
const pkg = JSON.parse(fs.readFileSync('package.json', 'utf8'))
if (!pkg.dependencies?.nodemailer) {
  console.log('⚠️  nodemailer not in package.json — run: npm install nodemailer @types/nodemailer')
} else {
  console.log('✅ nodemailer already installed')
}

console.log('\n🎉 Contribution system built! Now:')
...
console.log('3. git add . && git commit -m "feat: contribution/crowdfunding system" && git push origin main')
