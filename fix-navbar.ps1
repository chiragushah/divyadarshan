# DivyaDarshan - Fix Navbar encoding + Google Translate position
# Run from: C:\Users\chira\Downloads\divyadarshan
...
Write-Host " Fixing Navbar + Google Translate" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan

# ── STEP 1: Remove GoogleTranslate from layout.tsx ──────────
Write-Host "`n[1/3] Removing floating GoogleTranslate from layout.tsx..." -ForegroundColor Yellow

$layout = [System.IO.File]::ReadAllText("$PROJECT\app\layout.tsx")

# Remove the fixed-position floating div (line 73 area)
$layout = $layout -replace '<div style=\{\{position:"fixed",bottom:16,right:16,zIndex:9999\}\}><GoogleTranslate /></div>', ''

# Remove any bare <GoogleTranslate /> that remains
$layout = $layout -replace '\s*<GoogleTranslate />', ''

[System.IO.File]::WriteAllText("$PROJECT\app\layout.tsx", $layout, $utf8NoBOM)
Write-Host "   OK" -ForegroundColor Green

# ── STEP 2: Rewrite Navbar.tsx with correct encoding + GT ───
Write-Host "`n[2/3] Rewriting Navbar.tsx (fixing emojis + adding translate)..." -ForegroundColor Yellow
    ...
    <nav style={{ background: "#FFFFFF", borderBottom: "1.5px solid #E8E8E8", position: "sticky", top: 0, zIndex: 50, boxShadow: "0 1px 3px rgba(0,0,0,.04)" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px" }}>
        <div style={{ display: "flex", alignItems: "center", height: 72, gap: 8 }}>
          ...
          <div className="hidden md:flex" style={{ alignItems: "center", gap: 2, flex: 1 }}>
            <Link href="/manifest" style={{
              display: "flex", alignItems: "center", gap: 6, padding: "7px 12px",
              borderRadius: 9, background: "linear-gradient(135deg,rgba(139,26,26,0.08),rgba(192,87,10,0.08))",
              border: "1px solid rgba(139,26,26,0.15)", color: "var(--crimson)",
          ...
          {/* Desktop right side: Translate + Auth */}
          <div className="hidden md:flex" style={{ alignItems: "center", gap: 10, marginLeft: "auto" }}>

            {/* Google Translate — sits inline in navbar, no floating */}
            <GoogleTranslate />
      ...
      {mobileOpen && (
        <div style={{ background: "#FFFFFF", borderTop: "1.5px solid #E8E8E8", padding: "12px 24px 20px", maxHeight: "85vh", overflowY: "auto" }}>
          ...
          {/* Contribute */}
          <button
            onClick={() => { setMobileOpen(false); document.querySelector("[data-contribution-btn]")?.dispatchEvent(new MouseEvent("click")) }}
          ...
          {/* Language selector — Google Translate widget inline */}
...
[System.IO.File]::WriteAllText("$PROJECT\components\nav\Navbar.tsx", $navbar, $utf8NoBOM)
Write-Host "   OK" -ForegroundColor Green
...
git add "components\nav\Navbar.tsx"
git commit -m "fix: remove floating GoogleTranslate, add to Navbar inline, fix emoji encoding"
git push

Write-Host "`n============================================" -ForegroundColor Cyan
Write-Host " DONE - Vercel deploying in ~2 mins" -ForegroundColor Green
Write-Host " Google Translate now sits in the Navbar" -ForegroundColor Green
Write-Host " Emojis fixed throughout" -ForegroundColor Green
Write-Host "============================================" -ForegroundColor Cyan
