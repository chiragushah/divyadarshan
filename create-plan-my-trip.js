// create-plan-my-trip.js
// Creates all files needed for the Plan My Trip premium feature
const fs = require('fs')
...
// ── 1. TripPlanRequest Model ───────────────────────────────────────────────
ensureDir('models')
fs.writeFileSync('models/TripPlanRequest.ts', `import { Schema, model, models, Document } from 'mongoose'
...
`)
console.log('✅ models/TripPlanRequest.ts')

// ── 2. API Route ───────────────────────────────────────────────────────────
ensureDir('app/api/yatra/plan-request')
fs.writeFileSync('app/api/yatra/plan-request/route.ts', `import { NextRequest, NextResponse } from 'next/server'
...
`)
console.log('✅ app/api/yatra/plan-request/route.ts')

// ── 3. PlanMyTripModal Component ───────────────────────────────────────────
ensureDir('components')
fs.writeFileSync('components/PlanMyTripModal.tsx', `'use client'
...
`)
console.log('✅ components/PlanMyTripModal.tsx')

console.log('\n🎉 All files created!')
console.log('\nNext: Update app/(app)/plan/page.tsx to import and use PlanMyTripModal')
console.log('Run: node patch-plan-page-trip.js')
